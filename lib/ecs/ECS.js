    ...
    export() {
        ...
        for (const entity of this.entities) {
            ...
            for (const cr of entity[1].map) {
                if (!this.components.has(cr[1].constructor.name))
                    continue;
                components.push({
                    name: cr[0],
    ...
    load(save) {
        ...
        for (const entityRow of JSON.parse(save)) {
            ...
            for (const ComponentRow of entityRow.components) {
                if (!this.components.has(ComponentRow.name))
                    throw new Error(ComponentRow.name + " Component in save isn't registered.");
                const componentClass = this.components.get(ComponentRow.name);
                this.addComponent(entity, new componentClass(ComponentRow.data));
            }
        }
        if (this.debug)
            this.logger.debug("Done, " + ((performance.now() - start) / 1000).toString().slice(0, 5) + " S.");
    }
}
exports.default = ECS;
